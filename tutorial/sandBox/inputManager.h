...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		{
			if (scn->lastLinkId >= 0) { 
				Eigen::Vector4d tip_pos = scn->CalcParentsTrans(scn->lastLinkId) *
					scn->data_list[scn->lastLinkId].MakeTransd() *
					Eigen::Vector4d(scn->data_list[scn->lastLinkId].V.colwise().mean()[0],
						scn->data_list[scn->lastLinkId].V.colwise().mean()[1], scn->data_list[scn->lastLinkId].V.colwise().maxCoeff()[2], 1);
				std::cout << "tip position: (" << tip_pos.head(3).transpose() << ")" << std::endl;
		...
		case GLFW_KEY_UP:
			if (!scn->rotateScene)
				...
				scn->MyRotate(Eigen::Vector3d(1, 0, 0), 0.05);;
			//scn->direction = Eigen::Vector3d(0, 0.005, 0);
			//scn->shouldAnimate = !scn->shouldAnimate;
			//rndr->TranslateCamera(Eigen::Vector3f(0, 0.01f,0));
			break;
		case GLFW_KEY_DOWN:
			if (!scn->rotateScene)
				scn->data(scn->selected_data_index).MyRotate(Eigen::Vector3d(1, 0, 0), -0.05);
		...
		case ' ':
			...
			break;
		case '0':
			scn->data_list[scn->targetId].MyTranslate(O, true);
			break;
		case '1':
			scn->data_list[scn->targetId].MyTranslate(O+scn->data_list[scn->firstLinkId].GetRotation()* Eigen::Vector3d(0,0, scn->link_length/2), true);
		

			break;
		case '2':
			scn->data_list[scn->targetId].MyTranslate(O + scn->data_list[scn->firstLinkId].GetRotation() * Eigen::Vector3d(0, 0, scn->link_length / 2), true);
			scn->data_list[scn->targetId].MyTranslate(O + scn->data_list[scn->firstLinkId].GetRotation() * scn->data_list[2].GetRotation() * Eigen::Vector3d(0, 0, scn->link_length / 2), true);
	

			break;
		case '3':
			scn->data_list[scn->targetId].MyTranslate(O + scn->data_list[scn->parents[scn->parents[scn->firstLinkId]]].GetRotation() * Eigen::Vector3d(0, 0, scn->link_length / 2), true);
			scn->data_list[scn->targetId].MyTranslate(O + scn->data_list[scn->firstLinkId].GetRotation() * scn->data_list[2].GetRotation() * Eigen::Vector3d(0, 0, scn->link_length / 2), true);
			scn->data_list[scn->targetId].MyTranslate(O + scn->data_list[scn->firstLinkId].GetRotation() * scn->data_list[2].GetRotation() * scn->data_list[3].GetRotation() * Eigen::Vector3d(0, 0, scn->link_length / 2), true);
			break;
